...
[inputHandlers.ts](server/src/browser-management/inputHandlers.ts#L871-L912) `registerInputHandlers()` 注册所有**用户操作**相关的事件：
...
注册入口在 [connection.ts](server/src/socket-connection/connection.ts#L17-L25)：每次客户端连接时自动调用 `registerInputHandlers(socket, userId)`。

### 2.2 注册源 2：Generator.ts —— 工作流元控制

[Generator.ts](server/src/workflow-management/classes/Generator.ts#L191-L257) 注册**工作流控制**相关的事件，包括模式切换、列表抓取配置、保存等：
...
[RemoteBrowser.ts](server/src/browser-management/classes/RemoteBrowser.ts) 注册**纯浏览器同步**事件，这些事件**不经过 Generator，也不产生 workflow 条目**。
...
[DOMBrowserRenderer.tsx](src/components/recorder/DOMBrowserRenderer.tsx#L674-L728)
...
[RemoteBrowser.ts](server/src/browser-management/classes/RemoteBrowser.ts#L221-L235)
...
| 模式 | 代码位置 | 实现方式 | 现状 |
|------|---------|---------|------|
| **DOM 模式** | [DOMBrowserRenderer.tsx](src/components/recorder/DOMBrowserRenderer.tsx) | iframe + rrweb 事件流 + 前端直接监听 DOM 事件 | **当前默认启用** |
| **Screenshot 模式** | [legacy/src/Canvas.tsx](legacy/src/Canvas.tsx) | Canvas 渲染截图 + 坐标映射 + Playwright 生成选择器 | **已弃用，代码移至 legacy/** |
...
**遗留的服务端接口**：`Generator.ts` 中仍然保留了对应的 [onClick(coordinates, page)](server/src/workflow-management/classes/Generator.ts#L511) 方法，其核心逻辑：
...
- `isDOMMode` 初始值：服务端 `false`，但 [BrowserWindow.tsx L186-L194](src/components/browser/BrowserWindow.tsx#L186-L194) 中前端收到 `dom-mode-enabled` 后立刻回发确认，实际运行中默认启用 DOM 模式
- [BrowserWindow.tsx L2002-L2135](src/components/browser/BrowserWindow.tsx#L2002-L2135)：`isDOMMode ? <DOMBrowserRenderer /> : <DOMLoadingIndicator />`，非 DOM 模式只显示加载指示器
...
[DOMBrowserRenderer.tsx](src/components/recorder/DOMBrowserRenderer.tsx)
...
[inputHandlers.ts](server/src/browser-management/inputHandlers.ts)
...
[Generator.ts](server/src/workflow-management/classes/Generator.ts#L426-L458)
...
[Generator.ts](server/src/workflow-management/classes/Generator.ts#L295-L344)
...
|--------|---------|------|-----------|-------------|
| `dom:click` | [DOMBrowserRenderer.tsx L569](src/components/recorder/DOMBrowserRenderer.tsx#L569-L583) | `{ selector, userId, elementInfo, coordinates?, isSPA? }` | inputHandlers → Generator.onDOMClickAction | ✓ |
| `dom:keypress` | [DOMBrowserRenderer.tsx L642](src/components/recorder/DOMBrowserRenderer.tsx#L642-L647) | `{ selector, key, userId, inputType? }` | inputHandlers → Generator.onDOMKeyboardAction | ✓ |
...
|--------|-------|------|--------------|---------|-------------|
| `dom:scroll` | DOMBrowserRenderer wheelHandler | `{ deltaX, deltaY }` | [RemoteBrowser.ts L228-L234](server/src/browser-management/classes/RemoteBrowser.ts#L228-L234) | `page.mouse.wheel()` 同步滚动 | ✗ |
| `captureDirectScreenshot` | 前端截图按钮 | settings 对象 | [RemoteBrowser.ts L699-L701](server/src/browser-management/classes/RemoteBrowser.ts#L699-L701) | 执行截图并回发结果 | ✗ |
| `changeTab` | 前端标签页切换 | tabIndex: number | [RemoteBrowser.ts L703-L706](server/src/browser-management/classes/RemoteBrowser.ts#L703-L706) | 切换当前 page | ✗ |
| `addTab` | 前端新建标签页 | 无 | [RemoteBrowser.ts L708-L714](server/src/browser-management/classes/RemoteBrowser.ts#L708-L714) | `context.newPage()` + 切换 | ✗ |
| `closeTab` | 前端关闭标签页 | `{ index, isCurrent }` | [RemoteBrowser.ts L716-L728](server/src/browser-management/classes/RemoteBrowser.ts#L716-L728) | 关闭 page + 切换当前页 | ✗ |

### 6.4 服务端 → 前端（浏览器状态通知类）

| 事件名 | 发送位置 | 载荷 | 说明 |
|--------|---------|------|------|
| `rrweb-event` | [RemoteBrowser.ts L357](server/src/browser-management/classes/RemoteBrowser.ts#L357) | rrweb 事件对象 | DOM 模式实时页面渲染（核心数据通道） |
| `domLoadingProgress` | [RemoteBrowser.ts L237-L244](server/src/browser-management/classes/RemoteBrowser.ts#L237-L244) | `{ progress, pendingRequests, userId }` | 页面加载进度（0-100） |
| `dom-snapshot-loading` | [RemoteBrowser.ts L466](server/src/browser-management/classes/RemoteBrowser.ts#L466) | 加载状态 | DOM 快照加载中通知 |
| `urlChanged` | [RemoteBrowser.ts L265](server/src/browser-management/classes/RemoteBrowser.ts#L265) + [L921](server/src/browser-management/classes/RemoteBrowser.ts#L921) | `{ url, userId }` | 页面 URL 变化通知（导航/切标签页均触发） |
| `dom-mode-enabled` | controller 初始化成功后 | `{ userId }` | 通知前端 DOM 模式已就绪 |
| `dom-mode-error` | [controller.ts L50](server/src/browser-management/controller.ts#L50) | `{ userId, error }` | 通知前端 DOM 模式初始化失败 |
| `screenshotCaptureStarted` | [RemoteBrowser.ts L637](server/src/browser-management/classes/RemoteBrowser.ts#L637) | 状态信息 | 截图开始通知 |
| `directScreenshotCaptured` | [RemoteBrowser.ts L655](server/src/browser-management/classes/RemoteBrowser.ts#L655) | 截图数据 + metadata | 直接截图完成（返回 base64） |
| `screenshotError` | [RemoteBrowser.ts L629](server/src/browser-management/classes/RemoteBrowser.ts#L629) + [L664](server/src/browser-management/classes/RemoteBrowser.ts#L664) | 错误信息 | 截图失败通知 |

### 6.5 服务端 → 前端（工作流与 UI 类）

| 事件名 | 发送位置 | 载荷 | 说明 |
|--------|---------|------|------|
| `workflow` | [Generator.ts L342](server/src/workflow-management/classes/Generator.ts#L342) | `WorkflowFile` | 工作流更新通知（每次 pair 变化都发） |
| `highlighter` | [Generator.ts L1205-L1210](server/src/workflow-management/classes/Generator.ts#L1205-L1210) | `{ rect, selector, elementInfo, isDOMMode, shadowInfo }` | 元素悬停高亮数据 |
| `showDropdown` | [Generator.ts L546](server/src/workflow-management/classes/Generator.ts#L546) | `{ coordinates, selector, options }` | 显示下拉选择器弹窗 |
| `showDatePicker` | [Generator.ts L557](server/src/workflow-management/classes/Generator.ts#L557) | `{ coordinates, selector }` | 显示日期选择器弹窗 |
| `showTimePicker` | [Generator.ts L567](server/src/workflow-management/classes/Generator.ts#L567) | `{ coordinates, selector }` | 显示时间选择器弹窗 |
| `showDateTimePicker` | [Generator.ts L577](server/src/workflow-management/classes/Generator.ts#L577) | `{ coordinates, selector }` | 显示日期时间选择器弹窗 |
| `decision` | [Generator.ts L824](server/src/workflow-management/classes/Generator.ts#L824) | `{ pair, actionType, ... }` | 需要用户决策（如 over-shadowing 确认） |
| `fileSaved` | [Generator.ts L1077](server/src/workflow-management/classes/Generator.ts#L1077) + L1087 + L1120 + L1127 | `{ actionType }` | 工作流保存结果通知 |
| `newTab` | [Generator.ts L1241](server/src/workflow-management/classes/Generator.ts#L1241) | 页面标题 / 'new tab' | 新标签页通知 |
| `tabHasBeenClosed` | [Generator.ts L1237](server/src/workflow-management/classes/Generator.ts#L1237) | pageIndex | 标签页已关闭通知 |
| `paginationScrollTestResult` | [inputHandlers.ts L724](server/src/browser-management/inputHandlers.ts#L724) | 测试结果 | 分页滚动测试结果 |

### 6.6 工作流元控制（前端 → 服务端）

| 事件名 | 注册位置 | 说明 | 进入 workflow |
|--------|---------|------|-------------|
| `setGetList` | [Generator.ts L192](server/src/workflow-management/classes/Generator.ts#L192) | 设置列表抓取模式 | ✗ |
| `listSelector` | [Generator.ts L195](server/src/workflow-management/classes/Generator.ts#L195) | 指定列表容器选择器 | ✗ |
| `setPaginationMode` | [Generator.ts L198](server/src/workflow-management/classes/Generator.ts#L198) | 开启分页检测模式 | ✗ |
| `dom-mode-enabled` | [Generator.ts L204](server/src/workflow-management/classes/Generator.ts#L204) | DOM 模式确认（前端回发 ACK） | ✗ |
| `screenshot-mode-enabled` | [Generator.ts L209](server/src/workflow-management/classes/Generator.ts#L209) | Screenshot 模式切换（当前未使用） | ✗ |
| `save` | [Generator.ts L221](server/src/workflow-management/classes/Generator.ts#L221) | 保存工作流到数据库 | ✗（持久化） |
| `new-recording` | [Generator.ts L226](server/src/workflow-management/classes/Generator.ts#L226) | 重置工作流为空 | ✗ |
| `activeIndex` | [Generator.ts L231](server/src/workflow-management/classes/Generator.ts#L231) | 更新当前选中步骤索引 | ✗ |
| `decision` | [Generator.ts L232](server/src/workflow-management/classes/Generator.ts#L232) | 用户决策反馈 | ✗ |
| `updatePair` | [Generator.ts L254](server/src/workflow-management/classes/Generator.ts#L254) | 用户手动编辑 pair | ✗ |
...
| 文件 | 端 | 核心职责 |
|------|----|---------|
| [src/components/recorder/DOMBrowserRenderer.tsx](src/components/recorder/DOMBrowserRenderer.tsx) | 前端 | DOM 模式录制组件：iframe 事件监听 + **选择器生成** + Socket 发送 |
| [src/components/browser/BrowserWindow.tsx](src/components/browser/BrowserWindow.tsx) | 前端 | 浏览器窗口容器：DOM 模式切换、全局状态管理 |
| [src/helpers/clientSelectorGenerator.ts](src/helpers/clientSelectorGenerator.ts) | 前端 | 客户端选择器生成器：单例类 + @medv/finder 算法 + 分组算法 |
| [server/src/socket-connection/connection.ts](server/src/socket-connection/connection.ts) | 服务端 | Socket 连接入口：注册 inputHandlers |
| [server/src/browser-management/controller.ts](server/src/browser-management/controller.ts) | 服务端 | 浏览器控制器：创建/销毁 RemoteBrowser、DOM 模式初始化 |
| [server/src/browser-management/inputHandlers.ts](server/src/browser-management/inputHandlers.ts) | 服务端 | **注册源 1**：用户交互事件路由 + Playwright 动作执行 |
| [server/src/browser-management/classes/RemoteBrowser.ts](server/src/browser-management/classes/RemoteBrowser.ts) | 服务端 | **注册源 3**：滚动同步 + rrweb 录制 + 标签页/截图管理 |
| [server/src/workflow-management/classes/Generator.ts](server/src/workflow-management/classes/Generator.ts) | 服务端 | **注册源 2**：模式切换 + 工作流编排器：事件 → Where-What Pair |
| [server/src/workflow-management/selector.ts](server/src/workflow-management/selector.ts) | 服务端 | 服务端选择器生成（page.evaluate 注入，SDK/历史用） |
| [server/src/workflow-management/utils.ts](server/src/workflow-management/utils.ts) | 服务端 | 选择器优先级决策（多策略选优） |
| [legacy/src/Canvas.tsx](legacy/src/Canvas.tsx) | 前端（遗留） | 旧 Screenshot 模式：Canvas 截图 + 坐标映射 + input:mousedown 发送 |
